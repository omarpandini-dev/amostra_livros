...
export function ReaderControls(props: Props) {
  const { page, total, isAtEnd, muted, fullscreen, fullscreenSupported, onPrevious, onNext, onToggleMuted, onToggleFullscreen, onRestart } = props;
  ...
  return (
    <div className="reader-controls" aria-label="Controles de leitura">
      ...
      <button className={isAtEnd ? 'control finish-control' : 'control'} onClick={onNext} aria-label={isAtEnd ? 'Abrir o fim da aventura' : 'Próxima página'}><ArrowRight /><span>{isAtEnd ? 'Finalizar' : ''}</span></button>
